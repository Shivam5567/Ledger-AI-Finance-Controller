...
export function QuixoticTopNav({
  ...
  const tabs = [
    ...
    { id: 'transactions', label: 'Documents' },
...
// ── 2. Floating Left Pill Dock (matching vertical dock in reference) ───
export function QuixoticDock({ activeTab, onSelectTab, onToggleChat, onReload, isReloading }) {
  return (
    <aside className="hidden lg:flex flex-col gap-4 py-1 pr-3 select-none">
      {/* Pod 1: Primary Navigation */}
      <div className="pill-dock p-2 flex flex-col items-center gap-2">
        {/* Active green square button for Dashboard */}
        <button
        ...
        >
          <DashboardGridIcon className="w-5 h-5" />
        </button>
      ...
      <div className="pill-dock p-2 flex flex-col items-center gap-2">
        <button
          ...
          className="w-10 h-10 rounded-2xl flex items-center justify-center text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-all cursor-pointer"
        >
        ...
        <button
          ...
          className="w-10 h-10 rounded-2xl flex items-center justify-center text-gray-500 hover:text-[#007A4D] hover:bg-gray-100 transition-all cursor-pointer relative"
        >
          <ChatBubbleIcon className="w-5 h-5" />
          <span className="absolute top-2 right-2 w-2 h-2 rounded-full bg-[#007A4D]" />
        </button>
      ...
      <div className="pill-dock p-2 flex flex-col items-center gap-2">
        ...
        <button
          ...
          className="w-10 h-10 rounded-2xl flex items-center justify-center text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-all cursor-pointer disabled:opacity-40"
        >
          <ReloadIcon className={`w-5 h-5 ${isReloading ? 'animate-spin' : ''}`} />
        </button>
      </div>
    </aside>
  );
...
export function QuixoticHeaderRow({ onRunAgent, isRunning, txCount = 55 }) {
  return (
    <div className="flex flex-wrap items-center justify-between gap-4 py-2">
      ...
      {/* Right Controls: Date Range capsule + Action Button */}
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          onClick={onRunAgent}
          disabled={isRunning}
          className="flex items-center gap-2 bg-white hover:bg-gray-50 text-gray-800 border border-gray-200/90 rounded-full px-4 py-2 text-xs font-semibold shadow-xs hover:shadow transition-all cursor-pointer disabled:opacity-50"
        >
          {isRunning ? (
            <>
            ...
            <>
              <span className="text-[#007A4D] font-bold">+</span>
